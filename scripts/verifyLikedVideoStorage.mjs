...
try {
	...
	let duringWrite;
	const adapter = {
		exists: async p => { try { await access(resolveVaultPath(p)); return true; } catch (error) { if (error.code === 'ENOENT') return false; throw error; } },
		read: async p => { reads++; return readFile(resolveVaultPath(p), 'utf8'); },
		write: async (p, data) => { if (failWrite || (failPrimaryWrite && p.endsWith('/liked-videos.json'))) throw new Error('disk full'); writes++; await writeFile(resolveVaultPath(p), data); if (duringWrite) { const action = duringWrite; duringWrite = undefined; action(); } },
	};
	const values = new Map([[key, JSON.stringify([video])]]);
	const legacy = { getItem: k => values.get(k) ?? null, removeItem: k => values.delete(k) };
	let store = new LikedVideoStorageService(adapter, pluginDir);
	assert.equal(store.filePath, '.obsidian/plugins/geulo/liked-videos.json');
	...
	store = new LikedVideoStorageService(adapter, pluginDir);
	await store.initialize(legacy);
	...
	assert.deepEqual(JSON.parse(await readFile(resolveVaultPath(store.filePath), 'utf8')).videos, [video]);
	assert.deepEqual(JSON.parse(await readFile(resolveVaultPath(store.filePath + '.bak'), 'utf8')).videos, [video]);
	failPrimaryWrite = false;
	await store.flush();
	const restart = new LikedVideoStorageService(adapter, pluginDir);
	...
	for (const invalid of ['{broken', JSON.stringify({ schemaVersion: 2, videos: [] }), JSON.stringify({ schemaVersion: 1, videos: [{}] })]) {
		...
		assert.equal(await readFile(resolveVaultPath(store.filePath), 'utf8'), invalid);
		assert.ok(values.has(key));
